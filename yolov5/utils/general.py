    ...
    for k in 'train', 'val', 'test':
        if data.get(k):  # prepend path
            if isinstance(data[k], str):
                x = Path(os.path.join(str(path), data[k]))
                if not x.exists() and data[k].startswith('../'):
                    x = Path(os.path.join(str(path), data[k][3:]))
                data[k] = os.path.normpath(str(x))
            else:
                data[k] = [os.path.normpath(str(Path(os.path.join(str(path), str(p))))) for p in data[k]]
